...
export default function PropertyCard({ property }) {
  ...
  return (
    <Link to={`/properties/${id}`} className="group block">
      <div className={`overflow-hidden rounded-[1.6rem] border bg-white/88 shadow-[0_18px_40px_rgba(17,36,25,0.08)] transition-all duration-300 group-hover:-translate-y-1 group-hover:shadow-[0_24px_52px_rgba(17,36,25,0.14)] ${is_premium ? 'border-amber-200 ring-1 ring-amber-200/70' : 'border-white/70'}`}>
        <div className="relative h-56 overflow-hidden bg-gradient-to-br from-emerald-50 via-stone-100 to-emerald-100">
          {primaryPhoto ? (
            <img
              src={primaryPhoto}
              alt={title}
          ...
          <div className="absolute bottom-3 right-3 flex h-10 w-10 items-center justify-center rounded-full bg-white/90 text-slate-800 shadow-sm transition-transform duration-300 group-hover:translate-x-0.5 group-hover:-translate-y-0.5">
        ...
        <div className="p-5">
          <div className="mb-3 flex items-start justify-between gap-2">
            <h3 className="line-clamp-2 text-lg font-semibold text-slate-950 transition-colors group-hover:text-emerald-700">
              {title}
            </h3>
          </div>
          ...
          <div className="flex items-baseline justify-between gap-3 border-t border-slate-100 pt-4">
            ...
            <span className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">View</span>
          </div>
